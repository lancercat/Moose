...
from osocrNG.configs.typical_module_setups.base_model_NG import arm_base_training_modules,\
    arm_dom_fe,arm_fe_core,arm_dtd,arm_classifier,arm_base_prototyper,arm_imvn,arm_fe_base,\
    arm_stem_no_util,arm_dom_tatt
...
def arm_core_modules_hydra_sharenone(param,modcfgdict,bogocfgdict):
    ...
    modcfgdict, bogocfgdict = arm_stem_no_util(param, modcfgdict, bogocfgdict, prefix, save_path);
    ...
    for k in param["adict"]["names"]:
        backbone_core_name = prefix + k+ "_" + "backbone_core";
        modcfgdict, bogocfgdict = arm_fe_core(param, modcfgdict, bogocfgdict, prefix+k+"_", save_path);
        ...
        modcfgdict, bogocfgdict = arm_dtd(param, modcfgdict, bogocfgdict, prefix + k +"_", save_path);
        modcfgdict, bogocfgdict = arm_classifier(param, modcfgdict, bogocfgdict, prefix + k +"_", save_path);

        modcfgdict,bogocfgdict=arm_dom_tatt(param,modcfgdict, bogocfgdict, prefix+k+"_", save_path);

    return modcfgdict,bogocfgdict;

def get_hydra_v1_modules_sharenone(anchors,save_path,log_path,meta_path,decay=0.0005,expf=1):
    modset = neko_module_opt_setNG();
    modcfgs = {};
    bogocfgs = {};
    modcfgs, bogocfgs = arm_core_modules_hydra_sharenone(
        {
            "expf":expf,
            "adict":anchors,
...
def get_base_hydrav1_share_none(anchors,qprefix):
    rnams=anchors["names"];
    rdict={};
    ...
    for n in anchors["names"]:
        anchor_training_routine = get_tranining_routine_fpbp(n+"_", qprefix+n, n+"_pred", n+"_loss",
                                                           ...
                                                           n+"_"+"dtd", n+"_"+"classifier", "ocr_loss");
        rdict[n]=anchor_training_routine;
    return rnams,rdict;


def get_hydra_executor_core_share_none(params):
    d={
    ...
    for n in params["anchors"]:
        ...
        d["params"][n]=get_ocr_core(params["iocvt_dict"]["raw_image_name"], params["iocvt_dict"]["len_pred_argmax_name"],
                     ...
                     "mvn_core",  n + "_word_fe",  n + "_temporal_att", n+"_dtd", n+"_classifier");
    return d;

def get_test_hydra_routine_share_none(params):
    ...
    params["core"]=get_hydra_executor_core_share_none({
...
def arm_core_modules_hydra_shareall(param,modcfgdict,bogocfgdict):
    save_path = neko_get_arg("save_path", param);
    prefix=neko_get_arg("prefix",param,"");
    feat_ch = neko_get_arg("feat_ch", param, 512);
    expf=neko_get_arg("expf",param,1);
    param["feat_ch"] = feat_ch;
    modcfgdict,bogocfgdict=arm_imvn(param, modcfgdict, bogocfgdict, prefix, save_path)
    backbone_core_name = prefix+ "backbone_core";

    modcfgdict, bogocfgdict = arm_dom_fe({"backbone_core_name": backbone_core_name, "fe_name": prefix+"word_fe","expf":expf},
...
def get_hydra_v1_modules_shareall(anchors,save_path,log_path,meta_path,decay=0.0005,expf=1):
    modset = neko_module_opt_setNG();
    modcfgs = {};
    bogocfgs = {};
    modcfgs, bogocfgs = arm_core_modules_hydra_shareall(
        {
            "expf":expf,
            "adict":anchors,
            "dataroot": find_data_root(),
...
def get_base_hydrav1_share_all(anchors,qprefix):
    rnams=anchors["names"];
    rdict={};

    for n in anchors["names"]:
        anchor_training_routine = get_tranining_routine_fpbp(n+"_", qprefix+n, n+"_pred", n+"_loss",
                                                           "sampler", "mvn_core", "prototyper", "word_fe", "temporal_att",
                                                           "dtd", "classifier", "ocr_loss");
        rdict[n]=anchor_training_routine;
    return rnams,rdict;


def get_hydra_executor_core_share_all(params):
    d={
        "agent": neko_keyword_selective_execution_agent,
        "params": {
            "selector_name":params["iocvt_dict"]["selector_name"],
    ...
    for n in params["anchors"]:
        ...
        d["params"][n]=get_ocr_core(params["iocvt_dict"]["raw_image_name"], params["iocvt_dict"]["len_pred_argmax_name"],
                     ...
                     params["iocvt_dict"]["attention_map_name"],
                     params["iocvt_dict"]["feat_seq_name"],
                     "mvn_core",  "word_fe",  "temporal_att", "dtd", "classifier");
    return d;

def get_test_hydra_routine_share_all(params):
    prefix = neko_get_arg("prefix",params,"");
    ...
    params["modcvt_dict"]["prototyper_name"]=prefix+"prototyper";
    params["core"]=get_hydra_executor_core_share_all({
                "anchors":params["anchors"],
                "iocvt_dict": params["iocvt_dict"],
                "modcvt_dict": params["modcvt_dict"]
            });
